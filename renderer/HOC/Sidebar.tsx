...
function Sidebar({ children, title }: Props) {
  ...
  return (
    <>
      ...
      <div className="grid w-full h-screen min-h-screen grid-cols-12 bg-neutral-50">
        <div className="w-full h-full col-span-2 p-4 pt-24 bg-neutral-800">
          {navigation.map(({ name, path, icon }) => (
            <button
              className={clsx(
                "flex items-center w-full gap-4 px-4 py-2 mb-4 font-light rounded-md hover:bg-neutral-900",
                currentPath === path ? "text-lime-500" : "text-white"
              )}
            >
              {icon}
              <h4
